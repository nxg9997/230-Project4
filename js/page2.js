...
function RadiateTris(){
    let currMouseX = mousePos.x;
    let currMouseY = mousePos.y;
    let radCenter = new Victor(currMouseX, currMouseY);

    tagNum++;
    let newTag = "tri" + tagNum;

    let radArr = new Array(10);
    ...
    for (let i = 0; i < 10; i++){
        ...
        newTri.src = tri;
        newTri.alt = "tri";
        newTri.className = newTag;
        //triArr[i] = newTri;
        document.body.appendChild(newTri);
    }

    let classNm = "." + newTag;
    radArr = document.querySelectorAll(classNm);

    for (let j = 0; j < 10; j++){
        radArr[j].style.position = 'absolute';
        let newTop = radCenter.y - triHeight / 2;
        let newLeft = radCenter.x - triWidth / 2;
        radArr[j].style.top = "" + newTop + "px";
        radArr[j].style.left = "" + newLeft + "px";
        radArr[j].style.transformOrigin = "0% 0%";
    }

    for (let i = triNum; i < triNum + radArr.length; i++){
        triAccArr[i] = new Victor(0, 0);
        triVelArr[i] = new Victor(0, 0);
        triPosArr[i] = new Victor(50 * i, 50 * i);
    }
...
//triggers RadiateTris method when the user clicks the mouse
document.onmousedown = RadiateTris;
